...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const k = cap(key);
      let v = config[key];
      if (w <= 480 && c[`mobile${k}`]) v = c[`mobile${k}`];
      else if (w <= 768 && c[`tablet${k}`]) v = c[`tablet${k}`];
      else if (w <= 1024 && c[`desktop${k}`]) v = c[`desktop${k}`];
      setVal(v);
    };
...
const GradualBlur: React.FC<PropsWithChildren<GradualBlurProps>> = (props) => {
  ...
  const blurDivs = useMemo(() => {
    ...
    const curveFunc = CURVE_FUNCTIONS[config.curve] ?? ((p: number) => p);
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      if (config.exponential) {
        blurValue = Number(2 ** (progress * 4)) * 0.0625 * currentStrength;
      } else {
