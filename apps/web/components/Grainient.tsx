...
const Grainient: React.FC<GrainientProps> = ({
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    const { program } = ctx;
    // ogl types uniforms loosely; access them through `any` so per-key writes
    // type-check under the project's strict index settings.
    // biome-ignore lint/suspicious/noExplicitAny: ogl uniform map
